...
| `SEARXNG_BASE_URLS` | Self-hosted SearXNG instances (quota-free fallback; enable `format: json` in `settings.yml`) | Optional |
| `SEARXNG_PUBLIC_INSTANCES_ENABLED` | Discover public SearXNG instances from `searx.space` when no self-hosted URL is configured. Public nodes are frequently rate-limited or blocked, so this is opt-in and defaults to `false` | Optional |
