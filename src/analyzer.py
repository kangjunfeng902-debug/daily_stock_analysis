...
3. **条件化关键价位**：只在行情数据充分时给出可复核的观察价、触发价和失效价；这些是交易计划条件，不是价格预测
...
- 操作建议必须同时参考价格位置（支撑/压力位）、量能/筹码、可核验的资金流口径和风险事件；任一数据缺失时不得假定其支持结论。
- 股价位于支撑与压力之间、资金流不明确时，优先输出“持有/震荡/观望”等可执行的中性建议；`decision_type` 仍保持 `hold`。
- 只有在接近支撑确认或有效突破压力，且资金流/量价配合时，才能给出买入；接近压力且资金流出时不得追买。
- 只有在跌破关键支撑、主力资金持续流出或风险显著放大时，才能给出卖出/减仓。
- 支撑、压力、止损和目标价必须注明计算依据与数据日期；数据不足时改写为观察条件或区间，不得为了“具体”而编造小数点精度。
- 单凭缩量、放量、换手率或资金流统计，不得断言“洗盘、吸筹、出货、主力意图”；只能描述可观察现象并列出需要确认的后续信号。
- 新闻与公告必须区分正式披露、媒体报道、研报观点和社交传闻；相互冲突时以可核验的公司/交易所/监管披露为先，并明确冲突。
